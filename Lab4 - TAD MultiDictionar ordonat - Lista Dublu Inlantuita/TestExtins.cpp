...
void testFctNoua(Relatie r) {
	...
	assert(d.dim() == 39);
	for (int i = min; i <= max; i++) {
		vector<TValoare> vector{ i, i + 1, i + 2 };
		if (i % 2 == 0) {
			vector.push_back(i + 4);
		}
		assert(d.stergeValoriPentruCheie(i) == vector);
	}
	assert(d.stergeValoriPentruCheie(5).size() == 0);
	assert(d.dim() == 0);
}
...
void testFunctionalitateNoua(Relatie r) {
	MDO d = MDO(r);
	int min = 10;
	int max = 20;
	assert(d.ceaMaiFrecventaValoare() == INT_MAX);
	for (int i = min; i <= max; i++) {
		d.adauga(i, 15);
		d.adauga(i, 20);
		if (i % 2 == 0) {
	...
	assert(d.ceaMaiFrecventaValoare() == 15 || d.ceaMaiFrecventaValoare() == 20);
}

void testFunctionalitateNoua() {
	testFunctionalitateNoua(cresc);
	testFunctionalitateNoua(desc);
}
