...
void testFunctionalitateNoua() {
	...
	for (int i = 0; i < n - 1; i++) {
		Iterator ultimul = lo.ultimulIterator(i);
		ultimul.urmator();
		assert(ultimul.element() == i + 1);
	}
	Iterator ultimul = lo.ultimulIterator(4);
	assert(ultimul.element() == 4);
	ultimul.urmator();
	assert(ultimul.valid() == false);
	assert(lo.ultimulIterator(5).valid() == false);
}
